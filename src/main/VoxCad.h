...
class VoxCad : public QMainWindow {
  ...
  public slots: // global slot repository for updating things across all windows/views...
    ...
    void SaveAsZLib() {
        QString NewFileName;
        MainObj.SaveAsZLib(&NewFileName);
        SetWindowName(NewFileName);
    }
    void ImportVXA() {
        ForceViewMode();
        QString FileName;
        if (!MainSim.OpenVXA(&FileName))
            return;
        FastModeChanged(false);
        CheckNumVox();
        ZoomExtAll();
    ...
    void ImportHistory() {
        ForceViewMode();
        QString FileName;
        if (!MainSim.OpenHistory(&FileName))
            return;
        FastModeChanged(false);
        CheckNumVox();
        ZoomExtAll();
        ...
        UpdateAllWins();
        PhysicsMode(true);
        MainSim.pSimView->SetCurViewVox(RVV_HISTORY);
        MainSim.LockCoMToCenter = true;
    }
    void SaveVXA() {
        QString NewFileName;
        MainSim.SaveVXA(&NewFileName);
        SetWindowName(NewFileName);
    }
    ...
    // View
    void ViewPerspective(bool RedrawNow = true) {
        GLWindow->SetViewCustom1();
        if (RedrawNow)
            ReqGLUpdateAll();
    }
    void ViewTop(bool RedrawNow = true) {
        MainObj.SetV2DTop();
        GLWindow->SetViewTop();
        if (RedrawNow)
            ReqGLUpdateAll();
    ...
    void ViewBottom(void) {
        MainObj.SetV2DBottom();
        GLWindow->SetViewBottom();
        ReqGLUpdateAll();
    }
    void ViewLeft(void) {
        MainObj.SetV2DLeft();
        GLWindow->SetViewLeft();
        ReqGLUpdateAll();
    }
    void ViewRight(void) {
        MainObj.SetV2DRight();
        GLWindow->SetViewRight();
        ReqGLUpdateAll();
    }
    void ViewFront(void) {
        MainObj.SetV2DFront();
        GLWindow->SetViewFront();
        ReqGLUpdateAll();
    }
    void ViewBack(void) {
        MainObj.SetV2DBack();
        GLWindow->SetViewBack();
        ReqGLUpdateAll();
    }

    void ViewTriad(bool Visible) {}
    void FastModeChanged(bool Entering) {
        GLRef3DWin->SetFastMode(Entering);
        GLWindow->SetFastMode(Entering);
        ui.actionLarge_Mode->setChecked(Entering);
    }
    void ViewTiled(bool Tiled) {
        MainObj.SetTiledView(Tiled);
        ReqGLUpdateAll();
    }
    ...
    void CheckNumVox(void) {
        GraphicsEnabled = true;
        if (MainObj.GetNumVox() >= 1000000) {
            GraphicsEnabled = false;
            if (QMessageBox::question(NULL, "Disable Graphics?",
                                      "The selected file has greater than 1,000,000 voxels. Do you want to disable graphics?",
    ...
    void EditMode(bool entering = true) {
        if (entering)
            EnterVMMode(VM_EDITLAYER);
        else
            EnterVMMode(VM_3DVIEW);
    };
    void BCsMode(bool entering = true) {
        if (entering)
            EnterVMMode(VM_EDITBCS);
        else
            EnterVMMode(VM_3DVIEW);
    };
    void FEAMode(bool entering = true) {
        if (entering)
            EnterVMMode(VM_FEA);
        else {
            EnterVMMode(VM_3DVIEW);
    ...
    void RequestFEAMode(bool entering = true) {
        if (entering)
            MainFEA.RequestSolveSystem();
        else
            EnterVMMode(VM_3DVIEW);
    } // calls back with SolveSuccess signal
    void PhysicsMode(bool entering = true) {
        if (entering)
            EnterVMMode(VM_PHYSICS);
        else
            EnterVMMode(VM_3DVIEW);
    };
    void TensileMode(bool entering = true) {
        if (entering)
            EnterVMMode(VM_TENSILE);
        else
            EnterVMMode(VM_3DVIEW);
    };
    void Brush3DMode(bool entering = true) {
        if (entering)
            EnterVMMode(VM_3DBRUSH);
        else
            EnterVMMode(VM_3DVIEW);
    };
    ...
    void WantCoord3D(bool *YN) {
        if ((CurViewMode == VM_EDITLAYER && GLWindow->GetCurView() != VPERSPECTIVE) || CurViewMode == VM_PHYSICS)
            *YN = true;
        else
            *YN = false;
    }
    ...
    void LMouseDown(float X, float Y, float Z, bool IsCtrl) {
        if (CurViewMode == VM_EDITLAYER)
            MainObj.LMouseDown(Vec3D<>(X, Y, Z), IsCtrl);
        else if (CurViewMode == VM_PHYSICS)
            MainSim.LMouseDown(Vec3D<>(X, Y, Z));
    };
    void LMouseUp(float X, float Y, float Z) {
        if (CurViewMode == VM_EDITLAYER)
            MainObj.LMouseUp(Vec3D<>(X, Y, Z));
        else if (CurViewMode == VM_PHYSICS)
            MainSim.LMouseUp(Vec3D<>(X, Y, Z));
    };
    void LMouseDownMove(float X, float Y, float Z) {
        if (CurViewMode == VM_EDITLAYER)
            MainObj.LMouseDownMove(Vec3D<>(X, Y, Z));
        else if (CurViewMode == VM_PHYSICS)
            MainSim.LMouseDownMove(Vec3D<>(X, Y, Z));
    };
    ...
    void CtrlMouseRoll(bool Positive) {
        if (CurViewMode == VM_EDITLAYER)
            MainObj.CtrlMouseRoll(Positive);
    };

    // Hide and show dialog/windows (and keep checked stuff in sync!
    void ViewPaletteWindow(bool Visible) {
        if (Visible) {
            PaletteDockWidget->show();
            PaletteDockWidget->raise();
            ui.actionPalette->setChecked(true);
        } else {
            PaletteDockWidget->hide();
            ui.actionPalette->setChecked(false);
        }
    }
    void ViewWorkspaceWindow(bool Visible) {
        if (Visible) {
            WorkspaceDockWidget->show();
    ...
    void ViewRef3DWindow(bool Visible) {
        if (Visible) {
            Ref3DDockWidget->show();
            ui.actionReference_View->setChecked(true);
        } else {
            Ref3DDockWidget->hide();
            ui.actionReference_View->setChecked(false);
        }
    }
    void ViewVoxInfoWindow(bool Visible) {
        if (Visible) {
            VoxInfoDockWidget->show();
            VoxInfoDockWidget->raise();
    ...
    void ViewBCsWindow(bool Visible) {
        if (Visible) {
            ...
            BCsDlg->EditPrimCont.hide();
        }
    }
    void ViewFEAInfoWindow(bool Visible) {
        if (Visible) {
            FEAInfoDockWidget->show();
            FEAInfoDockWidget->raise();
            ui.actionSolve->setChecked(true);
        } else {
            FEAInfoDockWidget->hide();
            ui.actionSolve->setChecked(false);
        }
    }
    void ViewPhysicsWindow(bool Visible) {
        if (Visible) {
            PhysicsDockWidget->show();
            PhysicsDockWidget->raise();
            ui.actionPhysics->setChecked(true);
        } else {
            PhysicsDockWidget->hide();
            ui.actionPhysics->setChecked(false);
        }
    }
    void ViewTensileWindow(bool Visible) {
        if (Visible) {
            TensileDockWidget->show();
            TensileDockWidget->raise();
            ui.actionTensile->setChecked(true);
        } else {
            TensileDockWidget->hide();
            ui.actionTensile->setChecked(false);
        }
    }
    void ViewBrushWindow(bool Visible) {
        if (Visible) {
            BrushDockWidget->show();
            BrushDockWidget->raise();
            ui.action3D_Brush->setChecked(true);
        } else {
            BrushDockWidget->hide();
