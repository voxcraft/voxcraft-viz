...
public slots:
    void GetDim(Vec3D<> *pVec, Vec3D<> *pOff) {
        GetWorkSpace(pVec);
        *pOff = Vec3D<>(0, 0, 0);
    };
    ...
    void Save(int Compression = CP_ASCIIREADABLE, bool NewLoc = false,
              QString *pNewFilenameOut = NULL);
    void SaveZLib(void) { Save(CP_ASCIIREADABLE); };
    void SaveAsZLib(QString *pFilenameOut = NULL) {
        Save(CP_ASCIIREADABLE, true, pFilenameOut);
    };
    void SaveAsBase64() {
        Path = "";
        Save(CP_BASE64);
    };
    void SaveAsAsciiReadable() {
        Path = "";
        Save(CP_ASCIIREADABLE);
    };
    void ExportSTL(void);
    void ExportXYZ(void);
    void ExportKV6(void);
    ...
    void GetVXCInfoString(QString *pString) {
        std::string tmp;
        GetVXCInfoStr(&tmp);
        *pString = QString(tmp.c_str());
    };
    void GetVoxInfoString(int Index, QString *pString) {
        std::string tmp;
        GetVoxInfoStr(Index, &tmp);
        *pString = QString(tmp.c_str());
    };
...
private:
    QString GetLastDir() {
        return QSettings().value("CurrentDir", "").toString();
    }
    void SetLastDir(QString FullPath) {
        QString SelDir = QFileInfo(FullPath).path();
        QSettings().setValue("CurrentDir", SelDir);
    }
};
...
public slots:
    ...
    void SetViewDisplaced(bool View) {
        if (View)
            ViewDefPerc = 2.0;
        else
            ViewDefPerc = 0.0;
    ...
    void GetFEAInfoString(QString *pString) {
        std::string tmp;
        GetFEAInfoStr(&tmp);
        *pString = QString(tmp.c_str());
    };
    void GetFEAInfoString(int VoxIndex, QString *pString) {
        std::string tmp;
        GetFEAInfoStr(VoxIndex, &tmp);
        *pString = QString(tmp.c_str());
    };
...
public:
    //	QVX_Sim(CVX_Environment* pSimEnvIn, CMesh* pSimMeshIn = 0, QWidget
    //*parent = 0);
    ...
    bool LogEvery; // do we want to log every data point, or at fixed (real)
                   // time intervals?
    ...
    // reimplemented timestep and integration functions to multithread this baby
    // with QT threadpool! bool TimeStepMT(std::string* pRetMessage = NULL);
    // void IntegrateMT(IntegrationType Integrator = I_EULER);

    // Input voxel (moved by user interactively or otherwise
    void ApplySelection(int newSel) {
        CurXSel = newSel;
        if (newSel == -1)
            pVoxSel = NULL;
#ifdef VX2
            ...
            std::vector<CVX_Voxel *>::const_iterator it = voxels->begin();
            int counter = 0;
...
#endif
        // for (int i=0; i<newSel; i++) it++;
        //		pVoxSel = *(it);
    }
    CVX_Voxel *pVoxSel; // VX2
    int CurXSel;        // VX1 current selection... (VXC index...)
    ...
    int GLUpdateEveryNFrame; // updates GL window every N frames. -1 is off. If
                             // on, ensure StopExternalGLUpdate();

signals:
    void BCsChanged();
    void IsStatusTextVisible(bool *pIsVisible);
    void IsPlotVisible(bool *pIsVisible);
    void GetPlotRqdStats(char *pStatsRequired); // returns the stats the ui
                                                // needs (i.e. plot data)
    void ReqAddPlotPoint(double time);          // adds a point to the plot...
    void UpdateText(QString);
    void SimEndedInternally(
        QString); // emitted when simulation loop kicks out (unexpectedly)
...
public slots:
    ...
    void AutoSavePath(QString *pFilePath) {
        *pFilePath =
            VideoOutputFolder +
            QString::number(CurVideoFrameNumber).rightJustified(6, '0', false) +
            ".jpg";
        CurVideoFrameNumber++;
    }

public:
    //	void BondCalcForce(std::vector<CVXS_Bond>::iterator Bond)
    //{(*Bond).CalcForce();}
