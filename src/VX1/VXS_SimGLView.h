...
class CVXS_SimGLView : public QObject{
  ...
  public:
    ...
    void SetCurViewMode(ViewMode CurViewModeIn) {
        CurViewMode = CurViewModeIn;
        NeedStatsUpdate = true;
    }
    void SetCurViewCol(ViewColor CurViewColIn) {
        CurViewCol = CurViewColIn;
        NeedStatsUpdate = true;
    }
    ...
    CColor GetInternalBondColor(CVX_Link *pBond);
    ...
    // Drawing
    void DrawHistory(int Selected = -1, ViewVoxel historyView=RVV_HISTORY);
