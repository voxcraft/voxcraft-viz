...
{
    Camera(void){};
    Camera(float XPosIn, float YPosIn, float XRotIn, float YRotIn, float TargetXIn, float TargetYIn, float TargetZIn, float PerspIn,
           float ZoomIn) {
        XPos = XPosIn;
        YPos = YPosIn;
        XRot = XRotIn;
...
{
    GL_Light(void){};
    GL_Light(int LightInd, QColor &DifIn, QColor &SpecIn, Vec3D<> PosIn) {
        D[0] = DifIn.redF();
        D[1] = DifIn.greenF();
        D[2] = DifIn.blueF();
        D[3] = 1.0f;
...
class CQOpenGL : public QGLWidget {
  ...
  public slots:
    ...
    void StartAutoRedraw(int ms = 33) {
        AutoRedraw = true;
        DrawTimer->start(ms);
    }
    void StopAutoRedraw() {
  ...
  public:
    ...
    void GLSetBG(bool White = false) {
        if (White)
            GLSetBG(QColor(255, 255, 255));
        else
            GLSetBG(QColor(0, 0, 0));
    };
  ...
  public:                                                      // functions and variables only needed internally
    ...
    void GLScreenToCoord(int x, int y, float &pXD,
                         float &pYD); // for 2D views returns the world coordinates where we clicked, returns to pXD and pYD.
