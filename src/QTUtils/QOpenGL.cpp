...
CQOpenGL::CQOpenGL(const QGLFormat &format, QWidget *parent) : QGLWidget(format, parent) {
    ...
    MyID = TotalID++;
    //	if (format == 0) setFormat(QGLFormat(QGL::DoubleBuffer | QGL::DepthBuffer)); //This causes an assert on destruction in some cases.
    // switched to recomended passing in QGLFormat through constructor. 	setSizePolicy(2);
...
void CQOpenGL::paintGL() {
    ...
    if (!IniViewSet)
        GLCenterView(); // set the view appropriately the first time through
...
void CQOpenGL::mousePressEvent(QMouseEvent *event) {
    ...
    if (event->buttons() & Qt::LeftButton) {
        ...
        /*
                        ...
                        if (ExtScene2DProcess && CurView != VPERSPECTIVE){ //if its a 2D view we can process faster without using OpenGL
           (using signals...) float XCoord, YCoord; GLScreenToCoord(event->x(), event->y(), XCoord, YCoord);
    ...
    if (event->buttons() & Qt::LeftButton) { // If left mouse button is down:
        if (EmitCoord3D) {                   // if its a 2D view we can process faster without using OpenGL (using signals...)
            ...
            //			emit LMouseMovePressed(XC, YC);
               ...
               event->buttons() & Qt::RightButton) { // If middle mouse button is down: (manipulate the view!)
        ...
        if (event->modifiers() & Qt::ControlModifier) { // Control key down (translate)
            ...
            if (CurView == VPERSPECTIVE) { // if 3D view
                ...
                if (m_Cam.XRot > 360.0f)
                    m_Cam.XRot = 360.0f;
                if (m_Cam.XRot < 180.0f)
                    m_Cam.XRot = 180.0f;
        ...
        bool SavingFrames;
        emit WantAutoSaveFrames(&SavingFrames); // are we recording video?
        if (!AutoRedraw && !SavingFrames)
            updateGL();
        lastPos = event->pos();
...
void CQOpenGL::wheelEvent(QWheelEvent *event) {
    if (event->modifiers() & Qt::ControlModifier) { // if its a 2D view we can process faster without using OpenGL (using signals
        if (event->delta() > 0)
            emit CtrlWheelRoll(true);
        if (event->delta() < 0)
            emit CtrlWheelRoll(false);
    } else {
        if (event->delta() > 0)
            m_Cam.Zoom *= (float)1.05; // zoom zoom!
        if (event->delta() < 0)
...
void CQOpenGL::GLDrawScene() {
    ...
    case VTOP:
        glTranslated(0, 0, CurEnv.z);
        break;
    case VBOTTOM:
        glTranslated(0, 0, -CurEnv.z);
        break;
    case VLEFT:
        glTranslated(0, -CurEnv.y, 0);
        break;
    case VRIGHT:
        glTranslated(0, CurEnv.y, 0);
        break;
    case VFRONT:
        glTranslated(CurEnv.x, 0, 0);
        break;
    case VBACK:
        glTranslated(-CurEnv.x, 0, 0);
        break;
    ...
    if (MStoDraw > 200 && !IsFastMode() && !AskedAboutFastMode) {
        ...
        if (QMessageBox::question(NULL, "Enter fast draw mode?", "Do you want to enter fast drawing mode?",
...
void CQOpenGL::GLScreenToCoord3D(QPoint p, Vec3D<> &RetVec, bool SetZDepthHere) {
    ...
    glGetDoublev(GL_MODELVIEW_MATRIX,
                 modelMatrix); // unit matrix, but there shouldn't be much overhead in this call since we need for unproject() anyway
...
void CQOpenGL::GLScreenToCoord(int x, int y, float &pXD, float &pYD) // for 2D views returns the world coordinates where we clicked
{
    ...
    if (CurView == VPERSPECTIVE)
        return; // meaningless if we are not in an orthographic projection
    ...
    if (CurView == VFRONT || CurView == VBACK)
        XTarget = m_Cam.TargetY;
    float YTarget = m_Cam.TargetZ;
    if (CurView == VTOP || CurView == VBOTTOM)
        YTarget = m_Cam.TargetY;

    if (CurView == VBOTTOM || CurView == VRIGHT ||
        CurView == VBACK) // deal with the fact that the origin is on the left in negative axis views...
...
bool CQOpenGL::GLGetDim(Vec3D<> *pDim, Vec3D<> *pOff) {
    ...
    *pOff = Vec3D<>(0.0, 0.0, 0.0);
    ...
    if (pDim->x == 0.0 || pDim->y == 0.0 || pDim->z == 0.0)
        return false;
    return true;
}
...
void CQOpenGL::GLCenterView(bool ReZoom, Vec3D<> *pTargetPos) {
    ...
    if (pTargetPos)
        Target = *pTargetPos; // if we've passed a center point in the arguments, set to this
    else
        Target = CurEnvOff + CurEnv / 2;
    ...
    if (ReZoom && !CurSize.isNull()) {
        ...
        case VTOP:
        case VBOTTOM:
            m_Cam.Zoom = fmax(CurEnv.x / WinAspect, CurEnv.y) / 1.95;
            break;
        case VRIGHT:
        case VLEFT:
            m_Cam.Zoom = fmax(CurEnv.x / WinAspect, CurEnv.z) / 1.95;
            break;
        case VFRONT:
        case VBACK:
            m_Cam.Zoom = fmax(CurEnv.y / WinAspect, CurEnv.z) / 1.95;
            break;
        }
    }
}
...
void CQOpenGL::GLSetLighting() {
    if (FastMode) {
        ...
        for (int i = 0; i < (int)Lights.size(); i++) {
            Lights[i].SetLight(100 * fmax(CurEnv.x, fmax(CurEnv.y, CurEnv.z)), &CurEnvOff);
        }
...
void CQOpenGL::SetView(int ViewIndex) {
    if (CurView == (ViewType)ViewIndex)
        return; // don't reset a view (use GLCenterView if just want to see everything
    else
        CurView = (ViewType)ViewIndex;
...
void CQOpenGL::GLSaveScreenShot(void) { // save a screenshot of the scene
